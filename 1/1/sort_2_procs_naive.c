...
int main(int argc, char **argv)
{
    ...
    int div = floor((double) NUM_ELEMENTS / size);
    int rank_div;

    /* Check if last processor */
    if(size == rank+1)
        rank_div = NUM_ELEMENTS - (div*rank);
    else
        rank_div = div;
    ...
    if(rank == 0){
        ...
        /* Sort first Half */
        qsort(data, div, sizeof(int), compare_int);

        /* For More than 1 processor */
        for(i = 1; i < size; i++){
            if(i+1 == size) /* Check if last processor */
                rank_div = NUM_ELEMENTS - div*(size-1);
            /* Send block to sort */
            MPI_Send(&data[div*i], rank_div, MPI_INT, i, i, MPI_COMM_WORLD);
            /* Recieve sorted block */
            MPI_Recv(&data[div*i], rank_div, MPI_INT, i, i, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            /* Merge sorted blocks */
            merge(data, div*i, &data[div*i], rank_div);
        }
