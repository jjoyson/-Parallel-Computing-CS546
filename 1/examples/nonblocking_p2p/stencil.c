...
void free_bufs(double *aold, double *anew, double *atemp,
...
int main(int argc, char **argv)
{
    ...
    for (iter = 0; iter < niters; ++iter) {
        ...
        int replace = bx*by;
        int scountsx [replace];
        int scountsy [replace];
        int dis [replace];
        for(i = 0; i < size; i++){
            scountsx[i] = replace;
            dis[i] = 0;
        ...
        MPI_Alltoallv(aold, scountsx, dis, MPI_DOUBLE, atemp, scountsx, dis, MPI_DOUBLE, MPI_COMM_WORLD);
...
void alloc_bufs(int bx, int by, double **atemp_ptr, double **aold_ptr, double **anew_ptr,
                double **sbufnorth_ptr, double **sbufsouth_ptr,
                double **sbufeast_ptr, double **sbufwest_ptr,
                double **rbufnorth_ptr, double **rbufsouth_ptr,
                double **rbufeast_ptr, double **rbufwest_ptr)
{
    double *aold, *anew, *atemp;
    double *sbufnorth, *sbufsouth, *sbufeast, *sbufwest;
    double *rbufnorth, *rbufsouth, *rbufeast, *rbufwest;
    ...
    atemp = (double *) malloc(bx*by * sizeof(double));
    ...
    memset(atemp, 0, by * bx * sizeof(double));
                 ...
                 double *atemp)
{
    ...
    int j = 0;
    for (i = 0; i < bx; ++i){
        aold[ind(i + 1, 0)] = atemp[j];     /* #0 row */
        j++;
    }
    for (i = 0; i < bx; ++i){
        aold[ind(i + 1, by + 1)] = atemp[j];        /* #(by+1) row */
        j++;
    }
    for (i = 0; i < by; ++i){
        aold[ind(bx + 1, i + 1)] = atemp[j]; /* #(bx+1) col */
        j++
    }
    for (i = 0; i < by; ++i){
        aold[ind(0, i + 1)] = atemp[j];      /* #0 col */
        j++;
    }
}
