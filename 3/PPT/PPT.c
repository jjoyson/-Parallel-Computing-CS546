...
void lu_decomp(int n, int result, float * A, float ** B, int rank);
...
*/
    // STEP 3
    float **z = malloc(sizeof(float*) * 2*(size-1));
    float **v = malloc(sizeof(float*) * size);
    float **w = malloc(sizeof(float*) * size);
    float *x = malloc(sizeof(float*) * 2 * size);

    for(i = 0; i< size; i++){
        v[i] = malloc(sizeof(float) * 2);
        w[i] = malloc(sizeof(float) * 2);
    }

    for(i = 0; i < 2*(size-1); i++){
        z[i] = malloc(sizeof(float) * 2*(size-1));
    }
...
*/
    ...
    for(i = 0; i < size; i++){
        MPI_Bcast(&v[i][0], 1, MPI_FLOAT, i, MPI_COMM_WORLD);
        MPI_Bcast(&v[i][1], 1, MPI_FLOAT, i, MPI_COMM_WORLD);
        MPI_Bcast(&w[i][0], 1, MPI_FLOAT, i, MPI_COMM_WORLD);
        MPI_Bcast(&w[i][1], 1, MPI_FLOAT, i, MPI_COMM_WORLD);
        MPI_Bcast(&x[i*2], 1, MPI_FLOAT, i, MPI_COMM_WORLD);
        MPI_Bcast(&x[i*2+1], 1, MPI_FLOAT, i, MPI_COMM_WORLD);    
    }
...
*/
    ...
    float *h = malloc(sizeof(float) * 2*(size-1));
    memcpy(h, &x[1], sizeof(float) * 2*(size-1));
    free(x);

    temp_lu_decomp(2*(size-1), z, h);
    for(i = 0; i < 2*(size-1); i++){
        free(z[i]);
    }
    free(z);
...
*/
    ...
    for(int i = 0; i < m; i++){
        if(2*(rank -1 ) < 0){
            if(2*rank + 1 >= 2*(size-1))
                d_x[i] = D[1][i]*0 + D[2][i]*0;
            else
                d_x[i] = D[1][i]*0 + D[2][i]*h[2*rank+1];
        } 
        else{
            if(2*rank + 1 >= 2*(size-1))
                d_x[i] = D[1][i]*h[2*(rank-1)] + D[2][i]*0;
            else
                d_x[i] = D[1][i]*h[2*(rank-1)] + D[2][i]*h[2*rank+1];
        }
        d[i] =  D[0][i] - d_x[i];
    }
...
void lu_decomp(int n, int result, float * A, float ** D, int rank){
    int i,k;
    float L[n];
    float U[n];
/*
...
*/       
    for(i = 0; i < n; i++){
        if(i<1)
            ...
            L[i] = A[i*3+0]/U[i-1];
            U[i] = A[i*3+1]-L[i]*A[(i-1)*3+2];
        }

        for(int k = 0; k < result; k++){
            D[k][i] = D[k][i];
            if(i > 0)
                D[k][i] = D[k][i] - L[i]*D[k][i-1];
        }
    }
    ...
    for(i = n-1; i >= 0; i--){
        for(k = 0; k<result; k++){
            if(i > n-2)
                ...
                D[k][i] = (D[k][i] - A[i*3+2]*D[k][i+1])/U[i];
        }
    }
/*
    if(rank == 0){
